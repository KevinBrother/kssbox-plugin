...
- `./scripts/dev.sh biz boss`：启动 `biz` 和 `boss` 对应的前后端集合
...
对于复杂项目，外部接口仍然只有一个 `dev.sh`，但内部允许维护从服务别名到具体 app 的映射。例如：
...
- `.env.example`
- `docker/deploy.env.example`
- `docker/mirrors/aliyun.env`
- `docker/mirrors/tencent.env`
- `docker/mirrors/huawei.env`
- `docker/mirrors/volcengine.env`
...
除通用变量契约外，skill 默认提供一组可直接复制使用的国内云厂商镜像源模板，放在 `docker/mirrors/` 目录中。模板本身不写死到主脚本逻辑，而是作为“开箱即用示例”，方便用户按实际环境直接启用或二次覆盖。
...
1. `dev.sh` 参数解析正确
2. `dev.sh` 对 `all|biz|boss` 的服务展开正确
3. `dev.sh` 命中的启动命令正确
...
- 同时在 `docker/` 目录下生成 Dockerfile、Compose、环境变量示例与国内云厂商模板
